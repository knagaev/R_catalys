...
patent_path <- "D:/Downloads/Рефераты и формулы/Рефераты и формулы/"

patent_files <- list.files(patent_path)
...
for(i in 1:length(patent_files))
{
  print(patent_files[i])
  
  zip_name <- paste0(patent_path, patent_files[i])
  doc <- read_xml(unz(zip_name, filename = "PatentDocument.xml", open = "rb")) 
